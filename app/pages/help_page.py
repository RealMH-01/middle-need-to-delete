...
def _build_help_html() -> str:
    ...
    <ol>
      ...
      <li>每个文件夹旁边有一个<b>下拉标注框</b>，用来告诉程序这个文件夹是什么角色：
        <table>
          ...
          <tr><td><b>忽略</b></td><td>这个文件夹跟业务员无关，不导入</td></tr>
        </table>
      </li>
      <li>程序会根据文件夹下有没有子文件夹<b>自动判断默认标注</b>，
          大多数情况下不需要手动改。<br/>
          如果判断错了（比如把分公司当成了业务员），手动在下拉框里改一下就行。</li>
    ...
    新订单号 <code>HR-NEW001NH</code>。文件夹里有：<br>
    · <code>CI-HR-NEW001NH_对照.xlsx</code>（空白模板）→ 删除<br>
    · <code>CI-HR-OLD001NH.xlsx</code>（上一票文件，已改好）→ 程序识别为 CI 文件，
    重命名为 <code>CI-HR-NEW001NH.xlsx</code></div>
